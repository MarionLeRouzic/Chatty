<template>
  <q-form @submit="submitForm" class="form-auth">
    <q-input v-if="tab == 'register'" v-model="formData.name" class="q-mb-md" outlined label="Name" />
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions('storage', ['registerUser', 'loginUser']),
      submitForm() {
        if (this.tab === 'login') {
